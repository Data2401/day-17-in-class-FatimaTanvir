...
```{r}
ggplot(heights, aes(y = height, x = sex)) +
  geom_boxplot()
...
```{r}
votedata_2000 <- votedata %>% filter(year == 2000)

ggplot(votedata_2000, aes(x = state, y = demVote)) +
  geom_col()
```
...
ggplot(votedata_2000, aes(x = reorder(state, demVote), y = demVote, fill = south)) +
  geom_col() +
  coord_flip() +
  labs(title = "Democratic Vote Share by State in 2000",
       subtitle = "Ordered by Democratic Vote",
       x = "State",
...
ggplot(votedata_2000, aes(x = reorder(state, demVote), y = demVote, fill = south)) +
  ...
  labs(title = "Democratic Vote Share by State in 2000",
       ...
       x = "State",
       y = "Democratic Vote Share",
       fill = "Region (South)") + # Added a legend title
...
```{r}
votedata_08_16 <- votedata %>% filter(year %in% c(2008, 2016))

ggplot(votedata_08_16, aes(x = reorder(state, demVote), y = demVote, fill = south)) +
  geom_col() +
  coord_flip() +
  facet_wrap(~ year)
```
...
```{r}
map_plot_south_2008 <-  ggplot(data =  votedata %>% filter(year == 2008), aes(map_id = state)) +
  geom_map(aes(fill = south), map = states_map) +
  expand_limits(x = states_map$long, y = states_map$lat)
map_plot_south_2008

map_plot_south_2016 <-  ggplot(data =  votedata %>% filter(year == 2016), aes(map_id = state)) +
  geom_map(aes(fill = south), map = states_map) +
  expand_limits(x = states_map$long, y = states_map$lat)
map_plot_south_2016
```
